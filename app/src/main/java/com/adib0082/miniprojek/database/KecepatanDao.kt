...
interface KecepatanDao {
    ...
    @Query("SELECT * FROM kecepatan_sensor WHERE isDeleted = 0 ORDER BY waktu DESC")
    fun getAllActiveData(): Flow<List<SensorKecepatan>>

    // Ambil data dari Recycle Bin
    @Query("SELECT * FROM kecepatan_sensor WHERE isDeleted = 1 ORDER BY waktu DESC")
    ...
    @Query("UPDATE kecepatan_sensor SET isDeleted = 1 WHERE id = :id")
    suspend fun softDelete(id: Long)

    // Restore (Undo hapus)
    @Query("UPDATE kecepatan_sensor SET isDeleted = 0 WHERE id = :id")
